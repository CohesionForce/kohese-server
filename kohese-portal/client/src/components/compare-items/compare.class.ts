...
export class Compare {
  public static compareCommits(baseCommitId: string, changeCommitId: string,
    dynamicTypesService: DynamicTypesService): Array<Comparison> {
    let comparisons: Array<ItemProxyComparison> = [];
    let cache: ItemCache = TreeConfiguration.getItemCache();
    let baseTreeHashMap: TreeHashMap = cache.getTreeHashMap(baseCommitId);
    let changeTreeHashMap: TreeHashMap = cache.getTreeHashMap(changeCommitId);
    ...
    if (!diff.match) {
      for (let id in diff.details) {
        ...
        let baseItem: any = cache.getBlob(cache.getTree(diffEntry.
          treeHashChanged.fromTreeId).oid);
        if (!baseItem) {
          baseItem = {
            id: id,
        ...
        let changeItem: any = cache.getBlob(cache.getTree(diffEntry.
          treeHashChanged.toTreeId).oid);
        if (!changeItem) {
          changeItem = {
            id: id,
            name: 'Missing Item Version: ' + id
        ...
        comparison.adjustPropertyValue = (propertyValue: string, comparisonObject:
          any) => {
          ...
          if (uuidValueProperty) {
            if (treeHashMap[propertyValue]) {
              let item: any = cache.getBlob(treeHashMap[propertyValue].oid);
            ...
            if (uuidValueProperty) {
              ...
              for (let j: number = 0; j < elements.length; j++) {
                if (treeHashMap[elements[j]]) {
                  let item: any = cache.getBlob(treeHashMap[elements[j]].oid);
